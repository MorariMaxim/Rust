...
fn p1(path : &str)-> Result<(), io::Error>{
    
    let input = fs::read_to_string(path)?; 

    let mut oage = i32::MIN;
    let mut yage = i32::MAX;
    ...
    for line in input.lines() {
        ...
        if parts.len() != 3 {
            eprintln!("Invalid format in the file: {}", line);
            continue;
        }

        let name = parts[0].to_string();
        let phone_ = parts[1].to_string();
        let age_ = parts[2].parse::<i32>();
        let mut age = 0;
        if age_.is_ok()  {age= age_.unwrap()}
        else {continue;}
    ...
    if oldest.is_some() {
        ...
        println!("(The oldest student){:?}",oldest.unwrap());        
    }
    if youngest.is_some() {
        println!("(The youngest student){:?}",youngest.unwrap());
    }
...
fn new_canvas() -> Canvas {
    return Canvas { matrix: [[' ' as u8; 50]; 10] };
}

struct Canvas{
    matrix: [[u8; 50]; 10] 
}
fn set_pixels(canv : &mut Canvas, cord : &[(usize,usize,u8)]) {
    for i in cord {        
        canv.matrix[i.0][i.1] = i.2;
    }
} 
fn print(c : Canvas) {
...
fn p3(path : &str)-> Result<(), io::Error>{
    ...
    for line in input.lines() { 
        ...
        let stud : Student = res.unwrap();
        let age = stud.age;
        
        if age > oage {
            oage = age;
            oldest = Some(Student {
                name: stud.name.clone(),
                phone: stud.phone.clone(),
                age,
            });
        }
        if age < yage {
